	...
	if (cpu && (cpu->id_type() == 1 || cpu->id_type() == 2))
	{
		...
		if (cpu->id_type() == 2)
		{
			if (!g_tls_access_violation_recovered)
			{
				...
				vm_log.error("SPU Access violation %s location 0x%x (%s) [type=u%u]", is_writing ? "writing" : "reading", addr, (is_writing && vm::check_addr(addr)) ? "read-only memory" : "unmapped memory", d_size * 8);
			}
